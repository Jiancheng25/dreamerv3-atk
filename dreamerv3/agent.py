...
class Agent(embodied.jax.Agent):
  ...
  def policy(self, carry, obs, mode='train'):
    ...
    # Expose teacher distribution params for distillation
    for key, dist in policy.items():
      base = dist.output if hasattr(dist, 'output') else dist  # unwrap Agg
      if hasattr(base, 'logits'):            # Categorical
        out[f'logits/{key}'] = base.logits
      elif hasattr(base, 'dist') and hasattr(base.dist, 'logits'):  # OneHot
        out[f'logits/{key}'] = base.dist.logits
      if hasattr(base, 'mean'):              # Normal / bounded_normal
        out[f'mean/{key}'] = base.mean
      if hasattr(base, 'stddev'):
        out[f'stddev/{key}'] = base.stddev
